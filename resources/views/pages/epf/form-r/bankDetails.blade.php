<x-app-layout>
    <div style="padding:1rem; border: 2px dashed #e5e7eb; border-radius: 0.5rem;">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <h1 style="font-size: 1.25rem; font-weight: 600;">Historical Bank Details</h1>
        </div>

        <div class="flex gap-2 mb-4 mt-4">
            <a href="{{ route('epf.form-r.bankDetails.create') }}"
                class="bg-indigo-600 text-white px-4 py-2 rounded-md text-sm font-semibold">Add a New Bank Detail</a>
            <a href="{{ route('epf.form-r.index') }}"
                class="bg-red-600 text-white px-4 py-2 rounded-md text-sm font-semibold">Back to ETF list</a>
        </div>
                        ...
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="p-3 text-sm">
                                <div class="flex justify-end items-center gap-4">
                                    <a href="{{ route('epf.form-r.bankDetails.edit', ['month' => $record->month, 'year' => $record->year]) }}"
                                        class="text-indigo-600 hover:text-indigo-900 font-semibold">
                                        Edit
                                    </a>

                                    <form action="{{ route('epf.form-r.bankDetails.destroy', ['month' => $record->month, 'year' => $record->year]) }}"
                                        method="POST"
                                        onsubmit="return confirm('Are you sure you want to delete the bank details for {{ $record->month }} {{ $record->year }}?');">
                                        @csrf
                                        @method('DELETE')
