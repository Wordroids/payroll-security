    ...
    <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
        <div style="padding:2rem; border: 2px dashed #e5e7eb; border-radius: 0.5rem; background-color: white;">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h2 class="text-xl font-semibold text-gray-800">Add Monthly EPF Record (Form C)</h2>
                    <p class="text-sm text-gray-500">EPF Act No. 15 of 1958</p>
                </div>
                <a href="{{ route('epf.form-c.index') }}" class="text-sm text-indigo-600 hover:text-indigo-900">← Back to
                    List</a>
            </div>

            <form action="{{ route('epf.form-c.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">EPF Member No</label>
                        <input type="text" name="member_no" placeholder="e.g. 101" required
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-4 rounded-lg">
                    <div>
                        ...
                        <input type="number" name="total_earnings" id="total_earnings" step="0.01" required
                    ...
                    <div>
                        ...
                        <input type="number" name="surcharge" id="surcharge" step="0.01" value="0.00"
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                            oninput="calculateContributions()">
                    </div>
