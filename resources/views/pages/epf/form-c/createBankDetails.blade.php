<x-app-layout>
    <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
        <div style="padding:2rem; border: 2px dashed #e5e7eb; border-radius: 0.5rem; background-color: white;">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h2 class="text-xl font-semibold text-gray-800">
                        {{ isset($record) && $record ? 'Edit' : 'Add' }} Bank Details for {{ $month }}
                        {{ $year }}
                    </h2>
                    <p class="text-sm text-gray-500">Form C - Act No. 46 of 1980</p>
                </div>
                <a href="{{ route('epf.form-c.bankDetails') }}" class="text-sm text-indigo-600 hover:text-indigo-900">←
                    Back to Bank Details</a>
            </div>
            ...
            <form action="{{ route('epf.form-c.bankDetails.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="bank_name"
                            value="{{ isset($record) && $record ? $record->bank_name : '' }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="branch_name"
                            value="{{ isset($record) && $record ? $record->branch_name : '' }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="cheque_no"
                            value="{{ isset($record) && $record ? $record->cheque_no : '' }}" placeholder="e.g. 101"
                            required
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700">Cheque Return Charges</label>
                        <input type="number" step="0.01" name="cheque_return_charges"
                            value="{{ isset($record) && $record ? $record->cheque_return_charges : '0.00' }}"
