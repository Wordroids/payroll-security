                    ...
                    <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                        ...
                        <div class="overflow-vissible shadow-sm ring-1 ring-black/5 sm:rounded-lg">
                            <table class="min-w-full divide-y divide-gray-300">
                                ...
                                <tbody class="divide-y divide-gray-200 bg-white">@php
                                    $totalSalaryAdvancesFortheMonth = $salaryAdvances->sum('amount');
                                ...
                                @endphp
                                    ...
                                    @forelse ($employees as $employee)
                                        <tr>
                                            ...
                                            <td class="px-3 py-4 text-sm text-gray-700 whitespace-nowrap overflow-visible">
                                                <div class="flex items-center space-x-2 relative">
                                                    <span>Rs.{{ number_format($employee->salaryAdvances->sum('amount'), 2) }}</span>
                                            
                                                    <!-- Tooltip Trigger Icon -->
                                                    <svg data-tooltip-target="tooltip-default-{{ $employee->id }}" data-tooltip-placement="right"
                                                    ...
                                                    <!-- Tooltip Content -->
                                                    <div id="tooltip-default-{{ $employee->id }}"
                                                        role="tooltip"
                                                        ...
                                                        <ul class="space-y-2">
                                                            @foreach ($employee->salaryAdvances as $salaryAdvance)
                                                                <li class="border-b border-gray-600 pb-1">
                                                                    <span class="block font-semibold text-indigo-300">Rs{{ number_format($salaryAdvance->amount, 2) }}</span>
                                                                    <span class="block text-gray-300 text-xs">{{ \Carbon\Carbon::parse($salaryAdvance->advance_date)->format('Y-m-d') }}</span>
                                                                    <span class="block italic text-gray-400 text-xs">{{ $salaryAdvance->reason }}</span>
                                                                </li>
                                                            @endforeach
                                                        </ul>
